...
document.addEventListener("DOMContentLoaded", function () {
  function initServicePackage2(
    btnLeftelSelect,
    btnRightSelect,
    servicePackage2Select,
    servicePackage2SanpamSelect
  ) {
    ...
    // sản phẩm
    const servicePackage2Sanpam = document.querySelector(
      servicePackage2SanpamSelect
    );
    const styleservicePackage2Sanpam = window.getComputedStyle(
      servicePackage2Sanpam
    );
    ...
    const FullWidthSanpham =
      servicePackage2Sanpam.clientWidth + marginLeftSanpam + marginRightSanpam;
    ...
    function scrollleftRight() {
      if (
        scrollmousemove > scrollLeftwidthspham ||
        scrollmousemove <= scrollLeftwidthspham
      ) {
...
window.addEventListener("scroll", function () {
  requestAnimationFrame(() => {
    ...
    if (heroContent.getBoundingClientRect().top < window.innerHeight) {
      heroContent.style.transform = `translateY(${window.scrollTop / 8}px)`;
      // heroImg.style.transform = `translateY(${window.scrollTop / 8}px)`;
      herobg.style.transform = `translateY(-${window.scrollTop / 2}px)`;
    }
      ...
      itemtimgels.forEach(function (item) {
        ...
        if (HeightEl + HeightEltop > HeiWinDow) {
          item.style.transform = `translateY(${-(
            (HeightEl + HeightEltop - HeiWinDow) /
            1.5
          )}px)`;
        }
    ...
    //elementor-from
    // console.log(window.scrollTop - elementorFrom.getBoundingClientRect().top);
    if (elementorFrom.getBoundingClientRect().top < window.innerHeight) {
      elementorFrom.style.transform = `scale(1)`;
    }
    ...
    if (EssentialTips1text.getBoundingClientRect().top < window.innerHeight) {
      ...
      maybayjs.style.transform = `translateX(${
        EssentialTips1text.getBoundingClientRect().top
      }px)`;
    }
    // Essential-Tips 2
